 ...
 * 새 학교 추가
 */
function addSchool(name, code, scriptUrl) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['학교이름', '학교코드', '스크립트URL', '생성일']);
  }
  ...
  sheet.appendRow([name, code, scriptUrl, createdAt]);

  return {
    success: true,
    data: { name, code, scriptUrl, createdAt },
    message: '학교가 추가되었습니다.'
  };
}
 ...
 * 학교 정보 수정
 */
function updateSchool(originalCode, name, code, scriptUrl) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName('설정');
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][1] === originalCode) {
      ...
      return {
        success: true,
        data: { name, code, scriptUrl, createdAt: data[i][3] },
...
function doPost_admin(e) {
  ...
  switch (action) {
    ...
    case 'addSchool':
      result = addSchool(data.name, data.code, data.scriptUrl);
      break;

    case 'updateSchool':
      result = updateSchool(data.originalCode, data.name, data.code, data.scriptUrl);
      break;
...
function doPost(e) {
  ...
  switch (action) {
    ...
    case 'addSchool':
      return ContentService.createTextOutput(JSON.stringify(
        addSchool(data.name, data.code, data.scriptUrl)
      )).setMimeType(ContentService.MimeType.JSON);

    case 'updateSchool':
      return ContentService.createTextOutput(JSON.stringify(
        updateSchool(data.originalCode, data.name, data.code, data.scriptUrl)
      )).setMimeType(ContentService.MimeType.JSON);
